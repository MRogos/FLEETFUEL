...
router.post('/scan', upload.single('file'), async (req, res, next) => {
  try {
    ...
    const items = (parsed.items || []).map(item => {
      ...
      return { plate: item.plate, vehicle_id: vehicle?vehicle.id:null, vehicle_name: vehicle?vehicle.name:null, liters: parseFloat(item.liters)||null, net_amount: netPln, gross_amount: grossPln, price_per_l: item.price_per_l?parseFloat(item.price_per_l):null };
    });

    res.json({ ok: true, items, supplier, currency, eur_rate: eurRate });
  } catch(err) { next(err); }
});
